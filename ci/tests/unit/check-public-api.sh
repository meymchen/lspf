...
mkdir -p "$test_root/bin" "$test_root/ci" "$test_root/ci/policy"
cp ci/check-public-api.sh ci/supported-feature-matrix.sh "$test_root/ci/"
cp ci/policy/public-api-breaking-approvals.json "$test_root/ci/policy/"
...
jq -n \
    ...
    }]}' >"$test_root/ci/policy/public-api-breaking-approvals.json"
...
mv "$test_root/ci/approvals.tmp" "$test_root/ci/policy/public-api-breaking-approvals.json"
export FAKE_CURRENT_VERSION=0.5.2
if run_gate; then
