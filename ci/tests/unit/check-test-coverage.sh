...
mkdir -p "$test_root/ci" "$test_root/ci/policy" "$test_root/target"
...
cat >"$test_root/ci/policy/test-coverage-baseline.json" <<'EOF'
{
  "schemaVersion": 1,
  "policy": {
...
jq '{schemaVersion: 1, success: true, tests: .evidence}' \
    "$test_root/ci/policy/test-coverage-baseline.json" \
...
run_gate() {
    (
        ...
        bash ci/check-test-coverage.sh \
            ...
            --baseline ci/policy/test-coverage-baseline.json \
