...
workflow_yaml_to_json "$security_workflow" | jq -e '
  ...
  select(.run == "bash ci/tests/unit/check-repackaged-candidate.sh")
' >/dev/null

workflow_yaml_to_json "$security_workflow" | jq -e '
  .jobs["supply-chain"].steps[] |
  select(.run == "bash ci/tests/unit/prepare-release-record.sh")
' >/dev/null

workflow_yaml_to_json "$security_workflow" | jq -e '
  .jobs["supply-chain"].steps[] |
  select(.run == "bash ci/tests/workflow/release-publication.sh")
' >/dev/null
