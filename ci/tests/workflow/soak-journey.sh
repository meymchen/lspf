...
jq -e '
  ...
  and any(.steps[];
    ...
    and .run == "bash ci/tests/unit/run-soak-journeys.sh")
  and any(.steps[];
    .name == "Test soak workload contract"
    and .run == "bash ci/tests/system/soak-journeys.sh")
  and any(.steps[];
...
jq -e '
  .workflows[".github/workflows/ci.yml"]["bounded-memory-soak"]
    == {"contents":"read"}
' "$repo_root/ci/policy/workflow-permissions.json" >/dev/null

workflow_yaml_to_json "$repo_root/.github/workflows/security.yml" | jq -e '
  .jobs["supply-chain"].steps[]
  | select(.run == "bash ci/tests/workflow/soak-journey.sh")
' >/dev/null
