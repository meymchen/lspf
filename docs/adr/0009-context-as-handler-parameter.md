# Framework state reaches handlers via a `ServerContext` parameter

Every request [[Handler]] on the `LanguageServer` trait takes a
`ctx: &ServerContext` parameter in addition to `&self`, the request payload,
and the [[CancellationToken]]. `ServerContext` carries the [[Documents]]
...
We rejected a required `fn cx(&self) -> &ServerContext` getter (option B):
...
stored `ClientHandle` field (option D) because it conflates user state with
...
even when it's unused, and `ServerContext` will accumulate methods over time
