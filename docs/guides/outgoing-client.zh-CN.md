...
本指南介绍连接中从服务器到客户端的一侧：处理器通过 [`ServerContext`] 获得的带类型
[`ClientHandle`] 句柄、覆盖各项标准出站通知与请求的具名辅助方法、动态 capability
...
[`telemetry_event`](lspf::ClientHandle::telemetry_event) 和
[`progress`](lspf::ClientHandle::progress)，它们都会原样发送参数：
...
[`ServerContext::publish_diagnostics`](lspf::ServerContext::publish_diagnostics) 只是转发给 client
...
[`show_document`](lspf::ClientHandle::show_document) 与
[`show_message_request`](lspf::ClientHandle::show_message_request)，以及 workspace 交互
[`apply_edit`](lspf::ClientHandle::apply_edit)、
[`configuration`](lspf::ClientHandle::configuration) 与
[`workspace_folders`](lspf::ClientHandle::workspace_folders)。它们都是同一个带类型请求代理
...
[`register_capability`](lspf::ClientHandle::register_capability) 与
[`unregister_capability`](lspf::ClientHandle::unregister_capability) 在运行时向客户端声明
...
[`inline_value_refresh`](lspf::ClientHandle::inline_value_refresh) 和
[`semantic_tokens_refresh`](lspf::ClientHandle::semantic_tokens_refresh)。它们不接收参数，
并把客户端的 `null` 确认转换为 `()`。辅助方法不决定重新计算策略，框架也不保存可由
它们修改的 lens、diagnostic、hint、value 或 token 状态。

启用非默认 `proposed` Cargo feature 后，还会提供
[`refresh_folding_ranges`](lspf::ClientHandle::refresh_folding_ranges) 和
[`refresh_text_document_content`](lspf::ClientHandle::refresh_text_document_content)。因为
...
[`begin_progress`](lspf::ClientHandle::begin_progress) 以一个故障安全操作完成连接范围内的
...
marker type，再调用通用 [`request`](lspf::ClientHandle::request) 或
...
| `ClientHandle::workspace_folders` | `workspace/workspaceFolders` | 无（`null`） | `Option<Vec<WorkspaceFolder>>` | stable |
...
| `ClientHandle::code_lens_refresh` | `workspace/codeLens/refresh` | 无（`null`） | `()` | stable |
| `ClientHandle::diagnostic_refresh` | `workspace/diagnostic/refresh` | 无（`null`） | `()` | stable |
| `ClientHandle::inlay_hint_refresh` | `workspace/inlayHint/refresh` | 无（`null`） | `()` | stable |
| `ClientHandle::inline_value_refresh` | `workspace/inlineValue/refresh` | 无（`null`） | `()` | stable |
| `ClientHandle::semantic_tokens_refresh` | `workspace/semanticTokens/refresh` | 无（`null`） | `()` | stable |
| `ClientHandle::refresh_folding_ranges` | `workspace/foldingRange/refresh` | 无（`null`） | `()` | proposed |
...
| `ClientHandle::begin_progress` | `window/workDoneProgress/create`，然后发送 `$/progress` begin | `ProgressOptions` | `ProgressHandle` | stable |
