...
async fn hover(
    ...
    Ok(ctx.documents().get(&uri).map(|doc| Hover {
        contents: HoverContents::MarkupContent(MarkupContent {
            ...
            value: format!("{} words", doc.text(None).expect("full document text is always available").split_whitespace().count()),
        }),
        range: None,
    }))
}
...
async fn hover(
    ...
    Ok(Some(Hover {
        contents: HoverContents::MarkupContent(MarkupContent {
            kind: MarkupKind::PlainText,
            value: format!("{} words", document.text(None).expect("full document text is always available").split_whitespace().count()),
        }),
        range: None,
    }))
}
