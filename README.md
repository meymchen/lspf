...
> the full built-in outgoing helper coverage.
...
- **Complete helpers out of the box.** The complete set of
  outgoing notifications and requests ships as methods on `Context`
  ([ADR 0008](./docs/adr/0008-v1-scope-server-only-helper-coverage.md)).
...
- **0.4.x** — `worker_channel` transport for WASM-in-browser; full
  built-in outgoing helper coverage on `Context`.
