...
OUTPUT_PATH="${1:-.scratch/editor-validation-observations.md}"
...
stage "Install the packaged server"
say "One installed binary will be used by all three editors."
step "The wizard will install the current revision into target/editor-validation-install."
cargo install --path "$REPO_ROOT/crates/lspf-markdown" \
  --root "$INSTALL_ROOT" --locked --force
say "Installed server: $SERVER_PATH"
say "Evidence output: $OUTPUT_PATH"
pause "Press Enter after confirming the binary path."

stage "Visual Studio Code journey"
say "Close existing VS Code windows, then start Code from a shell with:"
...
stage "Zed journey"
say "Start Zed from the prepared shell, install the dev extension, and open the fixture."
...
say "Wrote human observations to $OUTPUT_PATH"
